...
const FlatList = () => {
    ...
    const actionSettings = {
        actions: {
            edit: { url: "/admin/edit-flat/", show: "1" },
            delete: { url: "", show: "1" },
        },
    };
    const addUrl = "/admin/add-flat";
    ...
    const fetchData = async (perPage = "", page = "", sortBy = "",search="",IsAsc="") => {
        ...
        try {
            ...
            const API_URL = appUrl + "/api/list-flat";
    ...
    const handleDelete = async (id) => {
        try {
            ...
            const appUrl = import.meta.env.VITE_API_URL;
            const API_URL = appUrl + "/api/delete-flat";
            const response = await axios.post(API_URL, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "society_id": `${society_token}`,
    ...
    const excelName = "flat_list";
    const excelApiUrl = "";
    return (
        <PageContainer
            title="Flat List"
            description="this is Flat List page"
        >
            <Breadcrumb title="" />
